...
// ---------------------------------------------------------------------------
// Shared by every visualization that supports TL playback.
// Individual visualizations compute their own bucket list from their own queries
// but subscribe to the same enabled/index/playing state so pressing ▶ Play
// animates them all in sync.
...
export const TimelapseProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const value = useMemo<TimelapseState>(() => ({
    ...
    reportBuckets, reportHotness, reportLoading,
  }), [enabled, bucket, bucketMs, speedMs, playing, index, totalBuckets, currentBucketKey, hotness, hotnessSource, isLoading, reportBuckets, reportHotness, reportLoading]);
