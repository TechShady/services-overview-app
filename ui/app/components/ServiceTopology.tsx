...
export function ServiceTopology({ edges: rawEdges, services, entityTypeMap, dbEntityMap, tfFrom = "now()-2h", tfTo = "now()", fromMs, toMs }: Props) {
            ...
            {tooltip.node.status === "PROBLEM" && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 10px", background: "rgba(194,25,48,0.1)", borderRadius: 6 }}>
                ...
                {tooltip.node.eventId ? (
                  <a
                    href={`${envUrl}/ui/apps/dynatrace.davis.problems/problem/${tooltip.node.eventId}?from=${encodeURIComponent(tfFrom).replace(/\(/g, '%28').replace(/\)/g, '%29')}&to=${encodeURIComponent(tfTo).replace(/\(/g, '%28').replace(/\)/g, '%29')}`}
                    target="_blank"
